...
function CopyButton({ code }) {
  ...
  return (
    ...
    >
      ...
      >
        <ClipboardIcon className="w-5 h-5 transition-colors fill-neutral-500/20 stroke-neutral-500 group-hover/button:stroke-neutral-400" />
        Copy
      </span>
      <span
        aria-hidden={!copied}
...
function CodeGroupHeader({ title, children, selectedIndex }) {
  ...
  return (
    <div className="flex min-h-[calc(theme(spacing.12)+1px)] flex-wrap items-start gap-x-4 border-b border-neutral-700 bg-neutral-800 px-4 dark:border-neutral-800 dark:bg-transparent">
      {title && <h3 className="pt-3 mr-auto text-xs font-semibold text-white">{title}</h3>}
      {hasTabs && (
...
export function CodeGroup({ children, title, ...props }) {
  ...
  return (
    <CodeGroupContext.Provider value={true}>
      <Container
        {...containerProps}
        className="my-6 overflow-hidden shadow-md not-prose rounded-2xl bg-neutral-900 dark:ring-1 dark:ring-white/10"
      >
